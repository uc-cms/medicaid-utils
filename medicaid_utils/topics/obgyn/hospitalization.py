...
def flag_smm_events(
    ...
    df_ip_claims["hosp_smm"] = df_ip_claims[
        df_ip_claims.columns[df_ip_claims.columns.str.startswith("hosp_smm_")]
    ].max(axis=1)

    df_ip_claims["hosp_smm_no_blood"] = df_ip_claims["hosp_smm"].where(
        (df_ip_claims["hosp_smm_transfusion"] == 0)
        | (
            df_ip_claims[
