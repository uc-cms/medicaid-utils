...
def flatten_nppes_query_result(pdf):
    ...
    ] = pdf["lst_matches"].apply(pd.Series)
...
def compute_match_purity(pdf):
    def match_purity(
        ...
        address_score = (
            0
            if (hclinic_street_address == "")
            else max(
                fuzz.ratio(
                    (p_loc_line_1 or "") + " " + (p_loc_line_2 or ""),
                    hclinic_street_address,
                ),
                fuzz.ratio(
                    (p_mail_line_1 or "") + " " + (p_mail_line_2 or ""),
                    hclinic_street_address,
                ),
        ...
        state_match = int(
            (hclinic_state == provider_id_state)
            or (hclinic_state == p_loc_state)
            or (hclinic_state == p_mail_state)
        )
    ...
    pdf[
        ["name_score", "address_score", "city_score", "state_match"]
    ] = pdf.apply(
        lambda row: match_purity(
            row["p_org_name"],
            row["p_org_name_oth"],
...
def compute_door_no_matches(pdf):
    pdf = pdf.assign(
        hclinic_address_digits=pdf["hclinic_street_address"]
        .apply(
            lambda x: next(
                iter([int(s) for s in x.split() if s.isdigit()]), np.nan
            )
        )
        .fillna(""),
    ...
    def parse_address(
        ...
        hclinic_usps_address = USPSAddress(
            name="",
        ...
        p_loc_usps_address = USPSAddress(
            name="",
        ...
        p_mail_usps_address = USPSAddress(
            name="",
    ...
    def match_purity(
        ...
        digits_match = (
            0
            if (hclinic_address_digits == "")
            else int(
                (hclinic_address_digits == p_loc_address_digits)
                or (hclinic_address_digits == p_mail_address_digits)
            )
        )
        door_no_match = (
            0
            if not bool(
                hclinic_post_box_id
                or hclinic_post_box_group_id
                or hclinic_address_no
                or hclinic_occupancy_no
            )
            else int(
                (
                    (
                        bool(hclinic_address_no)
                        & (
                ...
                & (
                    (
                        ...
                        & (
                            ...
                            | (hclinic_occupancy_no == p_mail_occupancy_no)
                        )
                    )
                    | (not bool(hclinic_occupancy_no))
                )
                & (
                    (
                        ...
                        & (
                            ...
                            | (hclinic_post_box_id == p_mail_post_box_id)
                        )
                    )
                    | (not bool(hclinic_post_box_id))
                )
                & (
                    (
                    ...
                    | (not bool(hclinic_post_box_group_id))
                )
            )
        )
        ...
        standardized_address_score = (
            ...
            if (
                ...
                fuzz.ratio(
                    p_loc_standardized_address, hclinic_standardized_address
                ),
                fuzz.ratio(
                    p_mail_standardized_address, hclinic_standardized_address
                ),
            )
        )
    ...
    pdf[
        ["standardized_address_score", "digits_match", "door_no_match"]
    ] = pdf.apply(
        lambda row: match_purity(
...
def match_hcris_fqhcs():
    ...
    df_hcris_matched = df_npi_provider.merge(
        df_hcris,
    ...
    df_hcris_matched_ploc = df_npi_provider.merge(
        df_hcris,
    ...
    df_hcris_matched_mloc = df_npi_provider.merge(
        df_hcris,
    ...
    df_hcris_matched = (
        pd.concat(
            [df_hcris_matched, df_hcris_matched_ploc, df_hcris_matched_mloc],
            ignore_index=True,
        )
        .reset_index()
    ...
    df_hcris_matched.to_pickle(
        os.path.join(fqhc_lookup_folder, "hcris_nppes_based_matches.pkl")
    )

    df_hcris_api_based = df_hcris.loc[
        ~df_hcris["hclinic_provider_id"].isin(
            df_hcris_matched["hclinic_provider_id"]
        )
    ]
    ...
    df_hcris_api_based[["result_count", "results"]] = df_hcris_api_based[
        "npi_json"
    ].apply(pd.Series)

    df_hcris_api_based_relaxed_city = df_hcris_api_based.loc[
        df_hcris_api_based["result_count"] < 1
    ]
    df_hcris_api_based_matched = df_hcris_api_based.loc[
        df_hcris_api_based["result_count"] >= 1
    ]

    df_hcris_api_based_relaxed_city = dd.from_pandas(
        df_hcris_api_based_relaxed_city, npartitions=30
    )
    df_hcris_api_based_relaxed_city = (
        df_hcris_api_based_relaxed_city.map_partitions(
            lambda pdf: pdf.assign(
                npi_json=pdf.apply(
                    lambda row: requests.get(
                        npi_api,
    ...
    ] = df_hcris_api_based_relaxed_city["npi_json"].apply(pd.Series)

    df_hcris_api_based_relaxed_name = df_hcris_api_based_relaxed_city.loc[
        df_hcris_api_based_relaxed_city["result_count"] < 1
    ]
    df_hcris_api_based_matched = pd.concat(
        [
            df_hcris_api_based_matched,
    ...
    df_hcris_api_based_relaxed_name = dd.from_pandas(
        df_hcris_api_based_relaxed_name, npartitions=30
    )
    df_hcris_api_based_relaxed_name = (
        df_hcris_api_based_relaxed_name.map_partitions(
            lambda pdf: pdf.assign(
                npi_json=pdf.apply(
                    lambda row: requests.get(
                        npi_api,
    ...
    df_hcris_api_based_relaxed_name[
        ["result_count", "results"]
    ] = df_hcris_api_based_relaxed_name["npi_json"].apply(pd.Series)

    df_hcris_api_based_relaxed_more_relaxed_name = (
    ...
    df_hcris_api_based_matched_flattened = flatten_nppes_query_result(
        df_hcris_api_based_matched
    )

    df_hcris_api_based_matched_flattened = dd.from_pandas(
        df_hcris_api_based_matched_flattened.replace(
            to_replace=[None, "None"], value=np.nan
        ).fillna(""),
        npartitions=30,
    )
    df_hcris_api_based_matched_flattened = (
        df_hcris_api_based_matched_flattened.map_partitions(
            lambda pdf: compute_match_purity(pdf)
        ).compute()
    )

    df_hcris_api_based_matched_flattened.to_pickle(
        os.path.join(fqhc_lookup_folder, "hcris_api_perfect_matches.pkl")
    )
    df_hcris_perfect_matches = (
        pd.concat(
    ...
    df_hcris_perfect_matches.to_pickle(
        os.path.join(fqhc_lookup_folder, "hcris_perfect_matches.pkl")
    )

    df_hcris_unmatched = df_hcris.loc[
        ~df_hcris["hclinic_provider_id"].isin(
            df_hcris_perfect_matches["hclinic_provider_id"]
        )
    ]
    df_hcris_state_relaxed = df_npi_provider.merge(
        df_hcris_unmatched,
        left_on=["provider_id"],
    ...
    df_hcris_state_relaxed = dd.from_pandas(
        df_hcris_state_relaxed.replace(
            to_replace=[None, "None"], value=np.nan
        ).fillna(""),
        npartitions=30,
    )
    df_hcris_state_relaxed = df_hcris_state_relaxed.map_partitions(
        lambda pdf: compute_match_purity(pdf)
    ).compute()

    df_hcris_state_relaxed = (
        df_hcris_state_relaxed.drop_duplicates(
    ...
    df_hcris_state_relaxed_matched = df_hcris_state_relaxed.loc[
        (df_hcris_state_relaxed[["name_score", "address_score"]] > 80).any(
            axis="columns"
        )
    ]
    ...
    df_hcris_matched = (
        pd.concat(
            [df_hcris_perfect_matches, df_hcris_state_relaxed_matched],
            ignore_index=True,
        )
        .sort_values(by=["hclinic_provider_id", "npi"])
        .reset_index(drop=True)
    ...
    df_hcris_matched.to_pickle(
        os.path.join(
            fqhc_lookup_folder, "hcris_perfect_matches_with_state_relaxed.pkl"
        )
    )
