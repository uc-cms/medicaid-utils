...
def combine_and_clean_hcris_files():
	...
	dct_col_names = {'prvdr_num': 'hclinic_provider_id',
	...
	lstdf_hcris = []
	for fname in glob.glob(os.path.join(hcris_lookup_folder, 'hclinic_provider_info_*.csv')):
		df_hcris = pd.read_csv(fname, dtype=object)
		df_hcris = df_hcris.rename(columns=dct_col_names)
		df_hcris = df_hcris[lst_col]
		df_hcris = df_hcris.assign(ccn=pd.to_numeric(df_hcris['hclinic_provider_id'].str.strip().str[-4:],
		                                             errors='coerce'))
		# Removing Rural Health Centers
		df_hcris = df_hcris.loc[df_hcris['ccn'].between(1800, 1989, inclusive='both') |
		                        df_hcris['ccn'].between(1000, 1199, inclusive='both')]
		df_hcris = df_hcris.drop(['ccn'], axis=1)
		lstdf_hcris.append(df_hcris)

	df_hcris = pd.concat(lstdf_hcris, ignore_index=True).reset_index(drop=True).drop_duplicates()
	df_hcris = df_hcris.assign(
		hclinic_provider_id_cleaned=df_hcris['hclinic_provider_id'].str.replace("[^a-zA-Z0-9]+", "",
		                                                                        regex=True).str.upper())
	df_hcris = df_hcris.assign(
		hclinic_provider_id_no_leading_zero=df_hcris['hclinic_provider_id_cleaned'].str.lstrip('0'))
	df_hcris.to_pickle(os.path.join(hcris_lookup_folder,
	                                'hcris_all_years.pickle'))
