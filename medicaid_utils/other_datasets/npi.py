...
def wide_to_long_nppes_taxonomy_file(year: int, pq_engine: str) -> None:
    ...
    :return: None
    """
    df_npi = dd.read_parquet(
        os.path.join(nppes_lookup_folder, str(year), "npi_cleaned_parquet"),
        engine=pq_engine,
        index=False,
    )
    ...
    def _mptn_wide_to_long(pdf, lst_id_col, lst_value_col, lst_nonnull_col):
        ...
        pdf = (
            pd.wide_to_long(pdf, lst_value_col, lst_id_col, "s_no", sep="_")
            .reset_index()
    ...
    lstlst_value_col = [
        ["p_tax_code", "p_lic_state", "p_prim_tax"],
    ...
    lstlst_id_col = [
        [col for col in lst if col in df_npi.columns] for lst in lstlst_id_col
    ]

    lsttpl_df = [
        (
            delayed(_mptn_wide_to_long)(
                df,
                lstlst_id_col[0],
                lstlst_value_col[0],
                lstlst_nonnull_col[0],
            ),
            delayed(_mptn_wide_to_long)(
                df,
                lstlst_id_col[1],
                lstlst_value_col[1],
                lstlst_nonnull_col[1],
            ),
            delayed(_mptn_wide_to_long)(
                df,
                lstlst_id_col[2],
                lstlst_value_col[2],
                lstlst_nonnull_col[2],
            ),
        )
        for df in df_npis
    ]
    ...
    dd.from_pandas(
        df_taxonomy, npartitions=max(ceil(n_rows / 50000), 1)
    ).to_parquet(
        os.path.join(nppes_lookup_folder, str(year), "npi_taxonomy_parquet"),
        engine="fastparquet",
        write_index=True,
    ...
    shutil.rmtree(
        os.path.join(nppes_lookup_folder, str(year), "npi_provider_parquet"),
        ignore_errors=True,
    )
    df_pid.to_parquet(
        os.path.join(nppes_lookup_folder, str(year), "npi_provider_parquet"),
    ...
    shutil.rmtree(
        os.path.join(
            nppes_lookup_folder, str(year), "npi_provider_parquet_cleaned"
        ),
        ignore_errors=True,
    )
    df_pid.to_parquet(
    ...
    shutil.rmtree(
        os.path.join(
            nppes_lookup_folder, str(year), "npi_taxonomy_group_parquet"
        ),
        ignore_errors=True,
    )
    dd.from_pandas(
        df_tax_group, npartitions=max(ceil(n_rows / 50000), 1)
    ).to_parquet(
        os.path.join(
            nppes_lookup_folder, str(year), "npi_taxonomy_group_parquet"
        ),
...
def generate_npi_taxonomy_mappings(
    year: int, pq_engine: str = "fastparquet"
) -> None:
    """
    ...
    pdf_taxonomy = pdf_taxonomy.assign(
        lst_cd=pdf_taxonomy["lst_cd"].str.split(",")
    )
    dct_taxonomy = pdf_taxonomy.set_index("taxonomy").T.to_dict("records")[0]

    df_npi_taxonomy = dd.read_parquet(
        os.path.join(nppes_lookup_folder, str(year), "npi_taxonomy_parquet"),
        engine=pq_engine,
        index=False,
    )
    ...
    df_npi_taxonomy = df_npi_taxonomy.rename(
        columns={"taxonomy_state": "licensing_state"}
    )
    df_npi_taxonomy = df_npi_taxonomy.loc[
        df_npi_taxonomy[lst_taxonomy_col].sum(axis=1) > 0
    ]

    df_npi_primary_taxonomy = df_npi_taxonomy.loc[
        df_npi_taxonomy["is_primary_taxonomy"].str.strip() == "Y"
    ].compute()
    df_npi_primary_taxonomy.to_pickle(
        os.path.join(
            nppes_lookup_folder, str(year), "npi_primary_taxonomies.pickle"
        )
    )
    ...
    df_npi_taxonomy.to_pickle(
        os.path.join(
            nppes_lookup_folder, str(year), "npi_secondary_taxonomies.pickle"
        )
    )
    ...
    lst_id_col = [
        col for col in lst_id_col if col in df_npi_taxonomy_group.columns
    ]
    df_npi_taxonomy_group = df_npi_taxonomy_group[
        lst_id_col + lst_taxonomy_col
    ]
    df_npi_taxonomy_group = df_npi_taxonomy_group.loc[
        df_npi_taxonomy_group[lst_taxonomy_col].sum(axis=1) > 0
    ].compute()
    df_npi_taxonomy_group.to_pickle(
        os.path.join(
            nppes_lookup_folder, str(year), "npi_taxonomy_groups.pickle"
        )
    )


def cleanup_raw_npi_files(
    lst_year: List[int], pq_engine: str = "fastparquet"
) -> None:
    """
    ...
    for year in lst_year:
        df_npi = dd.read_csv(
            os.path.join(nppes_lookup_folder, str(year), "npi-raw.csv"),
            dtype=object,
            blocksize="20MB",
        )
        df_npi = df_npi.replace(to_replace=[None], value=np.nan)
        df_npi = df_npi.rename(
            columns={
                **{
                    "NPI": "npi",
        ...
        shutil.rmtree(
            os.path.join(
                nppes_lookup_folder, str(year), "npi_cleaned_parquet"
            ),
            ignore_errors=True,
        )
        df_npi.to_parquet(
        ...
        if year > 2019:
            df_othname = dd.read_csv(
                os.path.join(
                    nppes_lookup_folder, str(year), "othname-raw.csv"
                ),
                dtype=object,
            )
            ...
            shutil.rmtree(
                os.path.join(
                    nppes_lookup_folder, str(year), "othname_cleaned_parquet"
                ),
                ignore_errors=True,
            )
            df_othname.to_parquet(
                os.path.join(
                    nppes_lookup_folder, str(year), "othname_cleaned_parquet"
                ),
                engine=pq_engine,
            ...
            df_ploc = dd.read_csv(
                os.path.join(
                    nppes_lookup_folder, str(year), "plocation-raw.csv"
                ),
                dtype=object,
            )
            ...
            shutil.rmtree(
                os.path.join(
                    nppes_lookup_folder, str(year), "plocation_cleaned_parquet"
                ),
                ignore_errors=True,
            )
            df_ploc.to_parquet(
