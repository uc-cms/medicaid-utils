...
class LowValueCare:
	...
	@classmethod
	def construct_low_value_care_measures(cls, pdf_dates, year, dct_msr_spec, dct_denom_spec):
		for idx, row in pdf_dates.iterrows():
			for service, measure in dct_msr_spec:
				...
				lst_msr_dates = row[f'msr_{service}_all_dates'] \
					if ((dct_service['exclude_ip'] != 1) & (dct_service['exclude_ed'] != 1)) \
				...
				for denom_condn in dct_service['denom_description']:
					...
					lst_denom_dates = row[f'denom_{denom_condn}_all_dates'] \
						if (((dct_denom_spec[(measure, denom_condn)]['exclude_ip'] != 1)
					...
					if bool(lst_denom_dates):
						if f'excl_chronic_{denom_condn}' in dct_service['denom_description']:
							lst_denom_dates = [min((x for x in lst_denom_dates if x.year == year),
							                       default=pd.to_datetime("", errors='coerce'))]
						else:
							if denom_condn.startswith('excl_chronic') and \
									(f"{denom_condn.replace('excl_chronic_', '')}" in dct_service['denom_description']):
								index_date = min((x for x in lst_denom_dates if x.year == year),
								                 default=pd.to_datetime("", errors='coerce'))
								lst_denom_dates = [x for x in lst_denom_dates
								                   if x < index_date]
						lst_denom_dates = [x for x in lst_denom_dates if pd.notnull(x)]
						if bool(lst_denom_dates):
							denom_prior_gap = dct_denom_spec[(measure, denom_condn)]['prior_days'] \
								if pd.notnull(dct_denom_spec[(measure, denom_condn)]['prior_days']) \
								else (dct_denom_spec[(measure, denom_condn)]['prior_months'] * 30
								      if pd.notnull(dct_denom_spec[(measure, denom_condn)]['prior_months'])
								      else srvc_followup_gap)
							denom_followup_gap = dct_denom_spec[(measure, denom_condn)]['followup_days'] \
								if pd.notnull(dct_denom_spec[(measure, denom_condn)]['followup_days']) \
								else (dct_denom_spec[(measure, denom_condn)]['followup_months'] * 30
								      if pd.notnull(dct_denom_spec[(measure, denom_condn)]['followup_months'])
								      else srvc_prior_gap)
							if pd.notnull(denom_prior_gap):
								lst_denom_dates = [denom_date for denom_date in lst_denom_dates
								                   if (len(row['eligibility_pattern'][:(
												denom_date.month + 12 * (denom_date.year - (year - 1)) - 1)]
								                           .replace('0', ''))
								                       >= (math.ceil(denom_prior_gap / 30)
								                           if pd.isnull(min_enrolled_months) else min_enrolled_months))]

							if pd.notnull(denom_followup_gap):
								lst_denom_dates = [denom_date for denom_date in lst_denom_dates
								                   if (len(row['eligibility_pattern'][(denom_date.month + 12 * (
												denom_date.year - (year - 1)) - 1):]
								                           .replace('0', ''))
								                       >= (math.ceil(denom_followup_gap / 30)
								                           if pd.isnull(min_enrolled_months) else min_enrolled_months))]

							offset = dct_denom_spec[(measure, denom_condn)]['offset_days'] \
								if pd.notnull(dct_denom_spec[(measure, denom_condn)]['offset_days']) \
								else dct_denom_spec[(measure, denom_condn)]['offset_months'] * 30
							if pd.isnull(offset):
								offset = 0
							if pd.notnull(index_date):
								lst_denom_dates = [denom_date for denom_date in lst_denom_dates
								                   if (index_date - denom_date).days <= denom_followup_gap - offset]

							if bool(lst_denom_dates):
								dct_denom_pop[denom_condn] = lst_denom_dates[-1]

							gap = denom_followup_gap if not (pd.isnull(denom_followup_gap)) else (-1 * denom_prior_gap)

							if pd.notnull(gap) & (gap < 0):
								offset = -1 * offset

							lst_denom_gaps = [((y - x).days, str(x), str(y)) for x, y in
							                  product(lst_denom_dates, lst_msr_dates)]

							if pd.notnull(gap):
								if (gap < 0):
									lst_matches = [x for x in lst_denom_gaps if (offset >= x[0] >= gap)]
									if bool(lst_matches):
										dct_denom_pop_msr[denom_condn] = lst_matches[0]
								if (gap > 0):
									lst_matches = [x for x in lst_denom_gaps if (offset <= x[0] <= gap)]
									if bool(lst_matches):
										dct_denom_pop_msr[denom_condn] = lst_matches[0]
								...
								if bool(lst_denom_dates) & bool(lst_msr_dates):
									dct_denom_pop_msr[denom_condn] = lst_denom_gaps[-1]
				pdf_dates.loc[idx, f"pop_{service}_denom_{measure}"] = 1 if (bool(dct_denom_pop) & (
					not any(condn for condn in dct_denom_pop if condn.startswith('excl_')))) else 0

				pdf_dates.at[idx, f'service_{service}_with_denom_{measure}'] = 1 if (
						bool(dct_denom_pop_msr) & (not any(condn for condn in dct_denom_pop
						                                   if condn.startswith('excl_')))) else 0
				pdf_dates.at[idx, f'dates_service_{service}_with_denom_{measure}'] = json.dumps(dct_denom_pop_msr) if (
						bool(dct_denom_pop_msr) & (not any(condn for condn in dct_denom_pop
						                                   if condn.startswith('excl_')))) else ""
		return pdf_dates
	...
	@classmethod
	def get_dates(cls, st, year, lst_condn, index_col, claims_folder):
		pdf_dates = None
		for ftype in ["ip", "ot"]:
			pdf = pd.read_parquet(
				os.path.join(claims_folder, ftype, f"{st}_{year}.parquet"),
				engine="fastparquet",
				index=False
			).set_index(index_col)
			pdf = pdf.rename(
	...
	def get_denom_measure_spec(cls):
		...
		dct_measures = pdf_combined_spec.groupby(['description', 'measure']).agg(
			dict([('measure', lambda x: x.values[0]),
			      ...
			      ('denom_description', list)])).to_dict(orient='index')

		dct_denom = pdf_denom_spec.set_index(['measure', 'description']).to_dict(orient='index')
		return dct_measures, dct_denom, lst_condn, pdf_denom_spec, pdf_measure_spec, pdf_combined_spec


def construct_low_value_care_measures(
	st, year, lst_bene_msis_filter, index_col, max_data_root, out_folder
):
	dct_measures, dct_denom, lst_condn, pdf_combined_spec, pdf_denom_spec, pdf_measure_spec = \
