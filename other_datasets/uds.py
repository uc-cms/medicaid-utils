...
def clean_uds_files(lst_year):
	...
	dct_col_names = {'UDS Reporting Year': 'reporting_year',
	...
	dct_site_info = {2011: 'GranteeSiteInfo',
	...
	dct_sheet_name = {2008: 'Grantee Level Data',
	...
	dct_filename = {2008: 'UDS_2008_DataDump_31Jul2009.xlsx',
	...
	for year in lst_year:
		df = pd.read_excel(os.path.join(uds_data_folder, dct_filename[year]),
		                   sheet_name=dct_sheet_name[year],
		                   dtype=object
		                   )
		df = df.rename(columns=dict([(col, dct_col_names[col]) for col in dct_col_names.keys() if col in df.columns]))
		if year in dct_site_info.keys():
			df_site = pd.read_excel(os.path.join(uds_data_folder, dct_filename[year]),
			                        sheet_name=dct_site_info[year],
			                        dtype=object
			                        )
			df_site = df_site.rename(columns=dict([(col,
			                                        dct_col_names[col]) for col in dct_col_names.keys() \
		...
		for col in lst_columns:
			if col not in df.columns:
				df[col] = np.nan
		pdf_uds = df[lst_columns] if pdf_uds is None else pd.concat([pdf_uds, df[lst_columns]], ignore_index=True)

		pdf_uds = pdf_uds.assign(**dict([(col,
		                                  (pdf_uds[col].fillna('False').astype(str).str.lower() == 'true').astype(int))
		                                 for col in pdf_uds.columns if col.startswith('funding_')]))
		pdf_uds = pdf_uds.assign(
			fqhc=pdf_uds[[col for col in pdf_uds if col.startswith('funding_')]].any(axis='columns').astype(
				int))

		pdf_uds.to_csv(os.path.join(uds_lookup_folder, f'uds_{lst_year[0]}_{lst_year[-1]}.csv'),
		               index=False)
