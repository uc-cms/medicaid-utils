...
def get_patient_ids_with_conditions(dct_diag_codes: dict, dct_procedure_codes: dict, logger_name=__file__,
	...
	for claim_type in dct_claims:
		...
		df = flag_diagnoses_and_procedures(dct_diag_codes, dct_procedure_codes, dct_claims[claim_type].copy())
		if df is not None:
			df['diag_condn'] = 0
			df['proc_condn'] = 0
			...
			if bool(dct_diag_codes):
				df = df.assign(diag_condn=df[[f'diag_{condn}' for condn in dct_diag_codes]].any(axis=1).astype(int))
	...
	if df_claims is not None:
		...
		if bool(dct_proc_codes):
			...
			lst_sys_code = list(set([int(sys_code) for lst_sys_code in
			                         [list(dct_proc_codes[proc].keys()) for proc in dct_proc_codes.keys()]
			...
			if n_prcdr_cd_col == 1:
				df_claims = df_claims.drop(["PRCDR_CD_1", 'PRCDR_CD_SYS_1'], axis=1)
			df_claims = df_claims.map_partitions(
				lambda pdf: pdf.assign(**dict([(f'proc_{proc}_{sys_code}',
				                                np.column_stack([pdf[col].str.startswith(
					                                tuple(dct_proc_codes[proc][sys_code]), na=False) for col in
					                                pdf.columns if
					                                col.startswith(
				                               ...
				                               [product([proc], list(dct_proc_codes[proc].keys()))
				                                for proc in dct_proc_codes] for proc, sys_code in sublist])))

			df_claims = df_claims.assign(**dict([(f'proc_{proc}',
			                                      df_claims[[col for col in df_claims.columns if
			                                                 col.startswith(f'proc_{proc}_')]
			                                      ].any(axis=1).astype(int)) for proc in dct_proc_codes.keys()]))
			df_claims = df_claims.drop([item for subitem in
			                            [[col for col in df_claims.columns if col.startswith(f'proc_{proc}_')]
