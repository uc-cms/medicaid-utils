...
.. list-table::
   ...
   * - **MAX**
     - Medicaid Analytic eXtract (pre-2016 file format)
   * - **TAF**
     - T-MSIS Analytic Files (2016+ file format)
   * - **T-MSIS**
...
.. list-table::
   ...
   * - Column
     - Description
   * - ``LST_DIAG_CD``
     - Comma-separated list of all diagnosis codes per beneficiary. MAX: must be
       constructed from ``DIAG_CD_*`` columns. TAF: created by
       ``gather_bene_level_diag_ndc_codes()`` on ``dct_files["base_diag_codes"]``
   * - ``LST_NDC``
     - Comma-separated list of all NDC codes per beneficiary. TAF: created by
       ``gather_bene_level_diag_ndc_codes()`` on ``dct_files["line_ndc_codes"]``
   * - ``ed_use``
   ...
   * - ``excl_missing_dob``
     - 1 if date of birth is missing (filter key: ``missing_dob``)
   * - ``excl_duplicated``
     - 1 if claim is a duplicate (filter key: ``duplicated``)
   * - ``ELX_GRP_1`` – ``ELX_GRP_31``
     - Elixhauser comorbidity group flags
